...
@Component({
  ...
  standalone: true,
  imports: [CommonModules, NzModules, TranslateModule, IconsProviderModule, ProfessionFormComponent],
...
export class ProfessionComponent implements OnInit {
  ...
  getAll() {
    this.loader = true;
    this.professionService.getAll(this.pageParams).subscribe((res: any) => {
      if (res && res.success) {
  ...
  add(): void {
    const drawerRef: any = this.drawer.create({
      ...
      nzContent: ProfessionFormComponent,
      nzPlacement: 'right',
    });
  ...
  remove(id: number | string) {
    this.confirmModal = this.modal.confirm({
      ...
      nzOkDanger: true,
      nzOnOk: () =>
        this.professionService.delete(id).subscribe((res: any) => {
          if (res && res.success) {
